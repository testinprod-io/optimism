...
func (g *OutputAsteriscGameHelper) CreateHonestActor(ctx context.Context, l2Node string, options ...challenger.Option) *OutputHonestHelper {
...
// TODO(pcw109550) modularize this
// type PreimageLoadCheck func(types.TraceProvider, uint64) error

func (g *OutputAsteriscGameHelper) CreateStepLargePreimageLoadCheck(ctx context.Context, sender common.Address) PreimageLoadCheck {
...
func (g *OutputAsteriscGameHelper) CreateStepPreimageLoadCheck(ctx context.Context) PreimageLoadCheck {
	return func(provider types.TraceProvider, targetTraceIndex uint64) error {
		execDepth := g.ExecDepth(ctx)
		_, _, preimageData, err := provider.GetStepData(ctx, types.NewPosition(execDepth, big.NewInt(int64(targetTraceIndex))))
		g.require.NoError(err)
...
func (g *OutputAsteriscGameHelper) ChallengeToPreimageLoad(ctx context.Context, outputRootClaim *ClaimHelper, challengerKey *ecdsa.PrivateKey, preimage asterisc.PreimageOpt, preimageCheck PreimageLoadCheck, preloadPreimage bool) {
	// Identifying the first state transition that loads a global preimage
	provider, _ := g.createAsteriscTraceProvider(ctx, "sequencer", outputRootClaim, challenger.WithPrivKey(challengerKey))
...
func (g *OutputAsteriscGameHelper) VerifyPreimage(ctx context.Context, outputRootClaim *ClaimHelper, preimageKey preimage.Key) {
	...
	// Identifying the first state transition that loads a global preimage
	provider, localContext := g.createAsteriscTraceProvider(ctx, "sequencer", outputRootClaim, challenger.WithPrivKey(deployer.TestKey))
	start := uint64(0)
	found := false
	for offset := uint32(0); ; offset += 4 {
		preimageOpt := asterisc.PreimageLoad(preimageKey, offset)
		...
		// TODO(pcw109550) add bindings for riscv; this is wrong
...
func (g *OutputAsteriscGameHelper) createAsteriscTraceProvider(ctx context.Context, l2Node string, outputRootClaim *ClaimHelper, options ...challenger.Option) (*asterisc.AsteriscTraceProviderForTest, common.Hash) {
	splitDepth := g.SplitDepth(ctx)
...
func (g *OutputAsteriscGameHelper) defaultChallengerOptions(l2Node string) []challenger.Option {
